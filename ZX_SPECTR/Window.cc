...
: _width(width), _height(height)
{
	_window = std::shared_ptr<SDL_Window>(
			SDL_CreateWindow("ZX_SPEC_EMUL", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_SHOWN), SDL_DestroyWindow);
	if (_window == nullptr) throw std::runtime_error(std::string("Window creation failed: ") + SDL_GetError());
	_renderer = std::shared_ptr<SDL_Renderer>(SDL_CreateRenderer(_window.get(), -1, SDL_RENDERER_ACCELERATED), SDL_DestroyRenderer);
	if (_renderer == nullptr) throw std::runtime_error(std::string("Renderer creation failed: ") + SDL_GetError());}
...
{
	switch(event.type) {
		case SDL_QUIT: _want_quit = true;
			break;
		case SDL_KEYDOWN:
			if (event.key.keysym.scancode == SDL_SCANCODE_ESCAPE) cpu.reset();
			if (event.key.keysym.scancode == SDL_SCANCODE_F6) cpu.save_state_sna("save.sna");
			if (event.key.keysym.scancode == SDL_SCANCODE_F9) cpu.load_state_sna("save.sna");
			if (event.key.keysym.scancode == SDL_SCANCODE_F8) cpu.load_state_sna("jetpac.sna");
...
{	unsigned bdr = io.border();
	...
	for (unsigned row = 0; row < 192; ++row) {
		for (unsigned col = 0; col < 32; ++col) {
			...
			unsigned raster_addr = 0x4000 | (a12a11 << 11) | (  a7a5 << 8) | ( a10a8 << 5) | (  a4a0 << 0);
			unsigned attr_addr = 0x5800 | (a12a11 << 8) | ( a10a8 << 5) | (  a4a0 << 0);
...
[[maybe_unised]]uint8_t flash = (paper_col >> 7) & 1;
			for (unsigned px = 0; px < 8; ++px) {
				SDL_Rect pixel { int(160 + PIXEL_SCALE * 8 * col + 3 * px), int(120 + PIXEL_SCALE * row), PIXEL_SCALE, PIXEL_SCALE };
				if ((1 << (7 - px)) & val)
					SDL_SetRenderDrawColor(_renderer.get(), red[ink_col], gre[ink_col], blu[ink_col], 255);
				else
					SDL_SetRenderDrawColor(_renderer.get(), red[paper_col], gre[paper_col], blu[paper_col], 255);
