...
{
	_context = z80ex_create(In_memo, this, Out_memo, this, In_inpout, this, Out_inpout, this, Int_read, this);}
Z80CPU::~Z80CPU()
{
...
{
	int steps = ticks;
	while (steps >= 0) steps -= z80ex_step(_context);}
void Z80CPU::reset()
{
...
void Z80CPU::save_state_sna(const char *filename)
{
	...
	uint16_t pc;
	uint8_t memory_reg;
	for (unsigned i = 0; i < ram.size(); i++) ram[i] = _ram.read(i);
...
void Z80CPU::load_state_sna(const char *filename)
{
	...
	for (unsigned i = 0; i < ram.size(); i++) _ram.write(i, ram[i]); _bus.write(0xfd, memory_reg, true); _bus.write(0xfe, hdr.FE, true);}
	...
	if (hdr1.stuffs1 & 0x20)
	{
		...
		do
		{
			...
			else
			{
				...
				if (b2 != 0xed)
				{	data[memptr] = b1; memptr++; data[memptr] = b2; memptr++; }
				else
				{z80f.read(reinterpret_cast<char *>(&xx), 1); z80f.read(reinterpret_cast<char *>(&yy), 1);
					while (yy > 0)
					{data[memptr++] = xx;}}}}
		while (z80f.good() and not z80f.eof());}
	else
	{ z80f.readsome(reinterpret_cast<char *>(&data[0]), data.size()); }
	for (uint16_t memptr = 0; memptr < data.size(); memptr++) _bus.write(memptr + 16384, data[memptr]);
	z80f.close();}
