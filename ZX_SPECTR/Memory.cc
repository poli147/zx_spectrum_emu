...
void AddressSpace::write(unsigned address, uint8_t value, bool io) {
	if (io == true) _io.write(address, value);
		...
		switch (seg) {
			case 0:
				break;
			case 1: _ram.write((_io.shadow() ? 7 : 5) << 14 | off, value);
				break;
			case 2: _ram.write(2 << 14 | off, value);
				break;
			case 3: _ram.write(_io.bank() << 14 | off, value);
				break;
			default:
				break;}}}
uint8_t AddressSpace::read(unsigned address, bool io) {
	if (io == true)
		...
		unsigned off = address & 0x3fff;
		switch (seg) {
			case 0:  return _rom.read(_io.rom() << 14 | off);
			case 1: return _ram.read((_io.shadow() ? 7 : 5) << 14 | off);
			case 2: return _ram.read(2 << 14 | off);
			case 3: return _ram.read(_io.bank() << 14 | off);
			default:
			    return 0;}}}
